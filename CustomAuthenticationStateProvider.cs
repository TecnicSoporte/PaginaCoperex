...
namespace PaginaWebCoperex.Services.LoginServices
{
    public class CustomAuthenticationStateProvider : AuthenticationStateProvider
    {
        ...
        public override async Task<AuthenticationState> GetAuthenticationStateAsync()
        {
            ...
            {
                // ✅ Prerendering activo — JS no disponible aún
                Console.WriteLine("⏳ Prerendering activo, retornando estado anónimo");
                return new AuthenticationState(_anonymous);
            }
            catch (Exception ex)
            {
        ...
        public async Task SetTokenAsync(string token)
        {
            ...
            {
                // ✅ Prerendering activo — JS no disponible aún
                Console.WriteLine("⏳ SetTokenAsync ignorado durante prerendering");
            }
            catch (Exception ex)
            {
        ...
        public async Task<string> GetTokenAsync()
        {
            ...
            {
                // ✅ Prerendering activo — JS no disponible aún, retornar vacío silenciosamente
                return string.Empty;
            }
            catch (Exception ex)
            {
        ...
        public async Task LogoutAsync()
        {
            ...
            {
                // ✅ Prerendering activo — JS no disponible aún
                Console.WriteLine("⏳ LogoutAsync ignorado durante prerendering");
            }
            catch (Exception ex)
            {
        ...
        private async Task RemoveTokenAsync()
        {
            ...
            {
                // ✅ Prerendering activo — JS no disponible aún
                Console.WriteLine("⏳ RemoveTokenAsync ignorado durante prerendering");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                if (expClaim != null)
                {
                    try
                    {
                        ...
                        Console.WriteLine($"⏰ Token expira: {expDateTime} UTC");
                        Console.WriteLine($"⏰ Hora actual: {DateTime.UtcNow} UTC");
                        ...
                        var horasRestantes = (expDateTime - DateTime.UtcNow).TotalHours;
                        Console.WriteLine($"✅ Token válido por {horasRestantes:F1} horas más");
                    }
                    catch (Exception ex)
                    {
                        Console.WriteLine($"⚠️ Error al parsear exp: {ex.Message}");
                    }
                }
                else
                {
                    Console.WriteLine("⚠️ No se encontró claim 'exp' en el token");
                }
